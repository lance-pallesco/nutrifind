...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        await api.startDemo();
        if (checkoutStatus === "cancelled") setNotice(labels.checkoutCancelled);
        const [recentData, account] = await Promise.all([api.recent(), api.me()]);
        setRecent(recentData.searches);
        setPremium(account.canViewNutrition);
        if (checkoutStatus === "success" && !account.canViewNutrition) {
          setNotice(labels.checkoutSuccess);
          for (let attempt = 0; attempt < 5; attempt += 1) {
            await new Promise((resolve) => setTimeout(resolve, 1500));
            const refreshed = await api.me();
            setPremium(refreshed.canViewNutrition);
            if (refreshed.canViewNutrition) {
              setNotice("");
              setResponse((current) => current ? { ...current, canViewNutrition: true } : current);
              break;
            }
          }
        }
      } catch { /* Search remains usable if account bootstrap is unavailable. */ }
    };
    void bootstrap();
  ...
  return <main className="min-h-screen overflow-hidden bg-[#f7faf8] text-slate-950">
    ...
    <header className="relative mx-auto flex w-full max-w-7xl items-center justify-between px-5 py-6 sm:px-8 lg:px-10">
      ...
      <div className="flex items-center gap-3"><span className={premium ? "hidden rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-bold text-emerald-700 sm:block" : "hidden rounded-full bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-500 sm:block"}>{premium ? labels.activeStatus : labels.basicStatus}</span><span className="hidden text-xs font-medium text-slate-400 md:block">{labels.demo}</span><div className="relative"><label htmlFor="language" className="sr-only">{labels.language}</label><select id="language" value={locale} onChange={(event) => setLocale(event.target.value as Locale)} className="appearance-none rounded-full border border-slate-200 bg-white py-2 pl-3 pr-8 text-xs font-semibold text-slate-700 shadow-sm outline-none transition focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100">{locales.map((item) => <option key={item} value={item}>{localeLabels[item]}</option>)}</select><ChevronDown className="pointer-events-none absolute right-2.5 top-2.5 size-3.5 text-slate-400" /></div></div>
    </header>
